...
const SearchOrder = () => {
    ...
    return (<>
        <h3>Mis pedidos</h3>
        <hr />
        <p className="search">Busca tu orden ingresando el id de tu orden.</p>
        <div className="d-flex row">
            <input className="search" type="text" placeholder="Ingresa el ID de tu orden" onChange={(evt) => setSearchId(evt.target.value)} />
        ...
        {itemsOrdered.length > 0 ? <div className="mt-5">
            ...
            : state && <div className="d-flex align-items-center mt-5"><i className="fas fa-exclamation-triangle fa-2x me-3" style={{color: "#dc3545"}} /><h4>El ID ingresado es incorrecto</h4></div>}</>
    )
}
